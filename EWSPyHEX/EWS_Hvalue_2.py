...
class EWS:
    ...
    def Expand(self, node,tt,th):
        ...
        if reslt != None:
            op_win,player_win,mustplay,scores = reslt[0],reslt[1],reslt[2],reslt[3]
            '''if op_win:
                tt.store(self.game.brd, Hex.oppCH(node.ptm))
            ...
            th.store(self.game.brd,[False,False,moves,scores])
        fullscore = len(scores)
        #print("Fullscore",fullscore,scores)
        for i in range(len(moves)):
            ...
            if winning:
                ...
                child = Node(m,Hex.oppCH(node.ptm),self.hweight*fullscore,self.hweight*(fullscore-score))
