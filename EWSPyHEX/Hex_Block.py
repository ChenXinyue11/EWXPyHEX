  ...
  for k in CELLS:
    if s[k]==ptm and k not in seen:
      ...
      while len(Q) > 0:
        ...
        for d in NBRS[c]:
          if d not in seen:
            if (s[d] == ptm and d >= 0) or d == s1 or d == s2:
              block.append(d)
              Q.append(d)
              seen.add(d)
      blocks.append(block)
...
class Position: # hex board 
    def __init__(self, rows, cols):
        ...
        for i in range(4):
            NBRS.append([])
        for c in range(COLS):
            point = coord_to_point(0, c, COLS)
            TOP_ROW.add(point)
            NBRS[point].append(-4)
            NBRS[-4].append(point)
        for r in range(ROWS):
            point = coord_to_point(r, 0, COLS)
            LFT_COL.add(point)
            NBRS[point].append(-3)
            NBRS[-3].append(point)
        for c in range(COLS):
            point = coord_to_point(ROWS-1, c, COLS)
            BTM_ROW.add(point)
            NBRS[point].append(-2)
            NBRS[-2].append(point)
        for r in range(ROWS):
            point = coord_to_point(r, COLS-1, COLS)
            RGT_COL.add(point)
            NBRS[point].append(-1)
            NBRS[-1].append(point)
