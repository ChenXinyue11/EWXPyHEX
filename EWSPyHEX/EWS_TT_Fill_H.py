...
class EWS:
    ...
    def SelectBackpropagate(self, node,tt):
        result = tt.lookup(self.game.brd)
        if result != None:
            if result == node.ptm:
                return True, True, 1, 1,None
        ...
        if child.expanded:
            isSolved, isWinning, wins, visits,wm = self.SelectBackpropagate(child,tt)
        else:
            isSolved, isWinning, wins, visits,wm = self.Expand(child,tt)
        self.game.undo()
        '''if VERBOSE:
            print("UNDO")
            self.game.Print()'''
    ...
    def Solve(self,ptm, timelimit = 1000):
        ...
        root = Node(None,ptm)
        isSolved, isWinning, wins, visits,wm = self.Expand(root,tt)
        if isSolved:
            return isWinning,wm,1
        isSolved, isWinning, _, _,wm = self.SelectBackpropagate(root,tt)
        iteration = 0
        while not isSolved:
