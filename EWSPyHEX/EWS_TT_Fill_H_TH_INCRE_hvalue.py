...
class Node:
    def __init__(self, move,ptm,hv = 0,hw = 0):
        self.ptm = ptm
        self.children = []
        self.move = move
        self.expanded = False
    ...
    def Value(self, pVisits):
        return (WR_WEIGHT / (WR_WEIGHT + pVisits)) * (self.ewLoss / (1 - (self.wins+self.hw) / (self.visits+self.hv)))\
            + (1 - (WR_WEIGHT / (WR_WEIGHT + pVisits))) * (1 / (1 - (self.wins+self.hw) / (self.visits+self.hv)))
...
class EWS:
    def __init__(self, game,hweight = 0):
    ...
    def Expand(self, node,tt,th,pstring = None,Ooldvcs = None,Ooldscvs = None,Poldvcs = None,Poldsvcs =None,Ooldhash = None,Poldhash = None):
        ...
        if reslt != None:
            node.pvcs,node.psvcs,node.Phash,node.Ovcs,node.Osvcs,node.Ohash,mustplay,scores = reslt[0],reslt[1],reslt[2],reslt[3],reslt[4],reslt[5],reslt[6],reslt[7]
            '''if op_win:
                tt.store(self.game.brd, Hex.oppCH(node.ptm))
            ...
            if mustplay == "nowin":
                ...
                for k in mustplay:
                    if self.game.brd[k]==Hex.ECH and k not in captured:
                        moves.append(k)
            moves,scores = nb.mustplay_order(moves,self.game.brd,pvcs,psvcs,node.ptm)
            th.store(self.game.brd,[pvcs,psvcs,phash,Ovcs,Osvcs,Ohash,moves,scores])
        fullscore = len(scores)
        ...
        for i in range(len(moves)):
            ...
            if winning:
                ...
                child = Node(m,Hex.oppCH(node.ptm),self.hweight*fullscore,self.hweight*(1+i))
