...
def increment(board,pboard, hashtable,color,samecolor, vcs, svcs):
    cells = []
    if not samecolor:
        color = Hex.oppCH(color)
    for cell in Hex.CELLS:
        if board[cell] == color and pboard[cell] == Hex.ECH:    
            cells.append(cell)
    if samecolor:
        #if the added stone is a key to svc, then the svc is become vcs
        for k,v in svcs.items():
            for e in v:
                #print("key",e.key)
                for stone in cells:
                    if e.key == stone:
                        carrier = e.carrier.copy()
                        if stone in e.carrier:
                            carrier.remove(stone)
                        vcs[k].add(VC(e.end1,e.end2,carrier,"incrementsvc"))
                        hashstring = vc_hash(False, e.end1, e.end2, carrier)
        ...
        for k,v in vcs.items():
            ...
            for e in v:
                for stone in cells:
                    if stone in e.end1 or stone in e.end2:
                        keyremove.add(k)
                    if stone in e.carrier:
                        vcsremove.add(e)
            for e in vcsremove:
...
def hsearch(board, color,turn,opt = 0,pboard = None,oldvcs = None,oldsvcs= None,oldhash = None,samecolor = None):
    if oldvcs == None:
        hashtable = set()
        vcs, svcs,hashtable = start(board, hashtable, color)
    else:
        vcs, svcs,hashtable = deepcopy(oldvcs), deepcopy(oldsvcs), deepcopy(oldhash)
        vcs, svcs,hashtable = increment(board,pboard,hashtable,color,samecolor,vcs,svcs)
    changes = True
    optm = Hex.oppCH(color)
...
t = time.time()
vcs, svcs,hash,_,_ = hsearch(p.brd, Hex.BCH, True, 1)
mustplay = mustplay_zone(p.brd,vcs,svcs,Hex.BCH)
print("mustplay:", mustplay)
string = p.brd
p.change_str(6, Hex.BCH)
Fill_in.fillin(p)
vcs, svcs,hash,_,_ = hsearch(p.brd,Hex.BCH,False,1)#,string,vcs,svcs,hash,True)
print("VCS:")'''
'''for k, v in svcs.items():
    if "-4" in k or "-2" in k:
        print(k)
