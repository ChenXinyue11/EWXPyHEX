...
def hsearch(board, color,turn,opt = 0):
    ...
    local_area = local.local_area(board, color, 2)
    changes = True
    optm = Hex.oppCH(color)
    ...
    while changes:
        ...
        for midstone in stones:
            for g1, g2 in combinations(stones, 2):
                ...
                if local_area != set():
                    if g1 not in local_area or g2 not in local_area or midstone not in local_area:
                        if len(vcs[end1 + end2]) >3 or len(svcs[end1 + end2]) >3:
                            continue
...
t = time.time()
vcs, svcs,hash,_,_ = hsearch(p.brd, Hex.BCH, False, 1)'''
'''print("VCS:")
for k, v in svcs.items():
    if "-4" in k and "-2" in k:
        print(k)
        for i in v:
            i.print()'''
